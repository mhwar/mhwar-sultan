...
import { Plus, Trash2, Check, X, TrendingUp, TrendingDown, Minus, Gauge } from 'lucide-react'
import { useShallow } from 'zustand/shallow'
import type { Project, Kpi, KpiTrend } from '@/types'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {kpis.map((k) =>
          editingId === k.id
            ...
            : <KpiCard key={k.id} kpi={k} onEdit={() => setEditingId(k.id)} onDelete={() => deleteKpi(k.id)} />
        )}
        {adding && (
          <KpiForm
...
function KpiCard({ kpi: k, onEdit, onDelete }: { kpi: Kpi; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <p className="text-sm font-medium" style={{ color: 'var(--color-text-secondary)' }}>{k.name}</p>
      ...
      {pct !== null && (
        <div className="mt-3">
          <div className="flex justify-between text-xs mb-1" style={{ color: 'var(--color-text-muted)' }}>
            ...
            <span className="num-tabular">{fmt(k.target!)} {k.unit}</span>
          </div>
...
function KpiForm({ initial, onSave, onCancel }: { initial?: Kpi; onSave: (d: Omit<Kpi, 'id' | 'order' | 'createdAt' | 'updatedAt' | 'projectId'>) => void; onCancel: () => void }) {
  ...
  const save = () => {
    if (!name.trim()) return
    onSave({ name, value: parseFloat(value) || 0, unit, target: target ? parseFloat(target) : undefined, trend: trend || undefined, notes: notes || undefined })
  }
