  ...
  const handleExport = () => {
    ...
    if (groupBy === 'none') {
      ...
      for (const t of exportTasks) {
        ...
        else if (groupBy === 'sprint') { key = t.sprintId ?? '__backlog'; label = t.sprintId ? (sprintNameMap[t.sprintId] ?? 'سبرنت') : 'الباكلوج' }
            ...
            <div className="flex gap-1.5 flex-wrap">
              {(['none', 'project', 'sprint', 'status', 'priority'] as GroupBy[]).map((g) => (
                <button key={g} onClick={() => setGroupBy(g)} className="px-3 h-7 rounded-full text-xs font-medium transition-colors" style={chipStyle(groupBy === g)}>
                  {g === 'none' ? 'بدون تجميع' : g === 'project' ? 'مشروع' : g === 'sprint' ? 'سبرنت' : g === 'status' ? 'الحالة' : 'الأولوية'}
                </button>
              ))}
            </div>
          </div>
